...
# Mirrors docker/tei-runpod/entrypoint.sh (log rotation + idle watchdog); also
# identical to docker/nli-runpod/entrypoint.sh except the fallback model name
# in the startup log line below (this image bakes in a different default).
set -euo pipefail
...
echo "  model: ${NLI_MODEL:-MoritzLaurer/bge-m3-zeroshot-v2.0-c}"
echo "  port:  ${NLI_PORT}"
