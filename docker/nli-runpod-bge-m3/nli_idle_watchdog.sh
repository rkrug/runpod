...
#
# Identical to docker/nli-runpod/nli_idle_watchdog.sh (the metrics contract
# is model-agnostic) — kept as its own copy per docker/CLAUDE.md's
# self-contained-per-image convention, not a shared file.
#
...
# Two distinct phases, because "has not been asked to do anything yet" and
# "has finished the work it was given" need very different timeouts.
#
# STARTUP phase — the pod has never served a request. The idle timer is NOT
# running. This is what makes bringing up a pool of pods safe: creating N pods
# takes N x CREATE_DELAY_SEC, they then boot concurrently, and the caller still
# has to collect their hostnames and point a client at them. Under the old
# single-timer behaviour the first pod's IDLE_MIN was already counting down
# throughout all of that -- it started the moment /metrics first answered -- so
# a large enough pool could not be brought up at all: the earliest pods stopped
# themselves before the last ones were ready. Note the health polling done by
# scripts/runpod/create_pods.sh does not help, because only POST /classify
# moves the counter watched here; GET /health and GET /metrics do not.
#
# The startup phase is bounded by STARTUP_GRACE_MIN so the failure mode it
# replaces (pod runs forever, billing, never used) is still covered -- and it
# now also covers a case the old logic missed entirely: if the model never
# loads, /metrics never answers, the old loop never advanced any counter and
# the pod ran until stopped by hand.
#
# IDLE phase — armed permanently by the first request. From then on the
# original behaviour applies: IDLE_MIN consecutive idle minutes stops the pod.
served=0
last_count=-1
idle_seconds=0
startup_seconds=0
...
while true; do
	...
	# "-1" is the unreachable sentinel, not a reading. Anything else from the
	# awk above is a non-negative integer; the case guard keeps a malformed
	# scrape from reaching an arithmetic test.
	case "${count}" in
	'' | *[!0-9]*) readable=0 ;;
	*) readable=1 ;;
	esac
	...
	if [ "${served}" -eq 0 ]; then
		...
		if [ "${readable}" -eq 0 ]; then
			echo "[idle-watchdog] server not answering yet (model still loading?); startup grace $((startup_seconds / 60))/${STARTUP_GRACE_MIN} min"
		else
			echo "[idle-watchdog] server up, no requests yet; startup grace $((startup_seconds / 60))/${STARTUP_GRACE_MIN} min"
		fi
		...
		if [ "${readable}" -eq 0 ]; then
			...
			echo "[idle-watchdog] metrics unreachable (server restarting?); idle timer held at $((idle_seconds / 60)) min"
		...
		# Guarded by > 0 so IDLE_MIN=0 really does disable the idle timer, as
		# this image's README has always documented. It did not: idle_seconds
		# starts at 0, and the old unguarded "-ge $((IDLE_MIN * 60))" was
		# therefore true on the very first poll, stopping the pod immediately.
