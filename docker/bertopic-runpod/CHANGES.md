...
Numbered v0.4.0 rather than the next unused minor: `bertopic-runpod:v0.3.0` is already
...
- **`STARTUP_GRACE_MIN` (new, default 60).** The idle watchdog now has two
  phases. Until the pod has been used at all -- the heartbeat being touched -- the
  ...
  The watchdog's own initialising `touch` no longer counts as use: its mtime
  is recorded at startup and only something newer arms the idle timer.
...
- **Fixed: a pod that never became usable ran forever.** When the heartbeat is only ever the one this watchdog writes at startup,
